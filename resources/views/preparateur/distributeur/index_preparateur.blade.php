			...
			<div class="page-wrapper">
				<div class="page-content">
					<div class="page-breadcrumb d-sm-flex align-items-center mb-2">
						...
						<div class="ps-3">Distributeurs</div>
			...
			$(".validate_pick_in").on('click', function(){
				...
				&& !localStorage.getItem('product_quantity_verif')){
					...
					$.ajax({
						...
						method: 'POST',
						data: {_token: $('input[name=_token]').val(), order_id: order_id, pick_items: pick_items, pick_items_quantity: pick_items_quantity, partial: false}
					}).done(function(data) {
			...
			$('body').on('click', '.valid_partial_order', function() {
				...
				if(pick_items){
					...
					$.ajax({
							url: "{{ route('orders.prepared') }}",
							method: 'POST',
							data: {_token: $('input[name=_token]').val(), order_id: order_id, pick_items: pick_items, pick_items_quantity: pick_items_quantity, partial: true, note_partial_order: note_partial_order}
					}).done(function(data) {
						if(JSON.parse(data).success){
							location.reload()
						} else {
