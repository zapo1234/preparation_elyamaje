												...
												<tr>
													...
													<td data-label="Status">
														@if($histo['order_status'] || $histo['order_dolibarr_status'])
															<select style="width: 180px; font-size: 0.9em;" data-from_dolibarr="{{ str_contains($histo['order_id'], 'BP') || str_contains($histo['order_id'], 'CO') ? 'true' : 'false' }}" data-order="{{ $histo['order_id'] }}" class="{{ $histo['order_status'] ?? $histo['order_dolibarr_status'] }} select_status select_user">
