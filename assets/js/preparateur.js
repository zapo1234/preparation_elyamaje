...
$(document).ready(function () {
        ...
        if (list_barcode) {
            Object.keys(JSON.parse(list_barcode)).forEach(function (k, v) {
                if (JSON.parse(list_barcode)[k]) {
                    ...
                    JSON.parse(list_barcode)[k].products.forEach(function (
                        item,
                        key
                    ) {
                        if (
                            typeof JSON.parse(list_barcode)[k].quantity !=
                            "undefined"
                        ) {
                            $("#order_" + order_id + " .barcode_" + item)
                                .find(".quantity_pick_in")
                                .text(
                                    JSON.parse(list_barcode)[k].quantity[key]
                                );
                            if (
                                parseInt(
                                    $(
    ...
    if (localStorage.getItem("product_quantity_verif")) {
        $(
            ".barcode_" + localStorage.getItem("product_quantity_verif")
        ).removeClass("pick");
    }
...
function progress_bar() {
    $(".modal_order").each(function () {
        ...
        $("#" + $(this).attr("id"))
            .find(".product_order")
            .each(function () {
                typeof quantity_to_pick_in_order[order_id] == "undefined"
                    ? (quantity_to_pick_in_order[order_id] = 0)
                    : quantity_to_pick_in_order[order_id];
                typeof quantity_pick_in_order[order_id] == "undefined"
                    ? (quantity_pick_in_order[order_id] = 0)
                    : quantity_pick_in_order[order_id];
                quantity_to_pick_in_order[order_id] =
                    quantity_to_pick_in_order[order_id] +
                    parseInt($(this).find(".quantity_to_pick_in").text());
                quantity_pick_in_order[order_id] =
                    quantity_pick_in_order[order_id] +
                    parseInt($(this).find(".quantity_pick_in").text());
            });

        progress[order_id] =
            (quantity_pick_in_order[order_id] * 100) /
            quantity_to_pick_in_order[order_id];

        if (progress[order_id] < 10) {
            $("#progress_" + order_id)
                .find(".progress-bar")
                .css("background-color", "#e62e2e");
            ...
            $("#progress_" + order_id)
                .find(".progress-bar")
                .css("background-color", "#3edd76");
        }

        $("#progress_" + order_id)
            .find(".progress-bar")
            .css("width", progress[order_id] + "%");
        $("#progress_" + order_id)
            .find(".progress-bar")
            .attr("aria-valuenow", progress[order_id]);
...
document.addEventListener("keyup", function (e) {
    ...
    if (
        ...
        !$("#infoMessageModal").hasClass("show") &&
        !$("#modalManuallyBarcode").hasClass("show") &&
        ...
        if (!isNaN(parseInt(e.key))) {
            ...
            if ($("#barcode").val().length == 13) {
                var quantity_to_pick_in = parseInt(
                ...
                if (
                    ...
                    if (
                        ...
                        quantity_pick_in =
                            quantity_pick_in > quantity_to_pick_in
                                ? quantity_to_pick_in
                                : quantity_pick_in;

                        if (
                            quantity_to_pick_in > 1 &&
                            quantity_to_pick_in <= 10 &&
                            parseInt(
                                $(
                                    "#order_" +
                                        order_id +
                            ...
                            setTimeout(function () {
                                $(".transfers_verif .close_modal").removeClass(
                                    "d-none"
                                );
                            }, 3000);
                            ...
                            $(
                                ...
                                .text(1);
                            $(
                                ".show .barcode_" + $("#barcode").val()
                            )[0].scrollIntoView({ behavior: "smooth" }, true);
...
$(".validate_pick_in").on("click", function () {
    var from_transfers =
        $(this).attr("from_transfers") == "1" ||
        $(this).attr("from_transfers") == "true"
            ? true
            : false;
    var order_id = $("#order_in_progress").val();
    var from_dolibarr =
        $(this).attr("from_dolibarr") == "1" ||
        $(this).attr("from_dolibarr") == "true"
            ? true
            : false;
        ...
        $.ajax({
            ...
            if (JSON.parse(data).success) {
                ...
                if (pick_items) {
                    ...
                    number_products = $("#order_" + order_id).find(
                        ".product_order"
                    ).length;
                }
                ...
                const href =
                    ...
                    number_products +
                    "," +
                    accentsTidy(customer_name) +
                    "," +
                    accentsTidy(user_name);
                const size = 150;
                    ...
                    Object.keys(pick_items).forEach(function (k, v) {
                        if (pick_items[k]) {
                            if (order_id == pick_items[k].order_id) {
                                pick_items.splice(
                                    pick_items.indexOf(pick_items[k]),
                                    pick_items.indexOf(pick_items[k]) + 1
                                );
                            }
                        }
                    });
...
function accentsTidy(r) {
    ...
    r = r.replace(new RegExp(/'/g), ""); // Supprime les apostrophes
    return r;
}
...
$(".confirmation_reset_order").on("click", function () {
    var order_id = $("#order_in_progress").val();
    var from_dolibarr =
        $(".validate_pick_in").attr("from_dolibarr") == "1" ||
        $(".validate_pick_in").attr("from_dolibarr") == "true"
            ? true
            : false;
    var from_transfers =
        $(".validate_pick_in").attr("from_transfers") == "1" ||
        $(".validate_pick_in").attr("from_transfers") == "true"
            ? true
            : false;
...
$("body").on("click", ".valid_partial_order", function () {
    ...
    $.ajax({
        ...
        data: {
            ...
            pick_items_quantity: pick_items_quantity,
            partial: 1,
            note_partial_order: note_partial_order,
            from_dolibarr: from_dolibarr,
            from_transfers: from_transfers,
        },
    }).done(function (data) {
    ...
    if (localStorage.getItem("barcode")) {
        ...
        if (pick_items.length > 0) {
            Object.keys(pick_items).forEach(function (k, v) {
                if (pick_items[k]) {
                    if (order_id == pick_items[k].order_id) {
                        Object.keys(pick_items[k].products).forEach(function (
                            l,
                            w
                        ) {
...
$(".valid_manually_barcode").on("click", function () {
    ...
    $.ajax({
        ...
        if (JSON.parse(data).success) {
            if (
                !$("#order_" + order_id + " .barcode_" + barcode).hasClass(
                    "pick"
                )
            ) {
                $("#order_" + order_id + " .barcode_" + barcode).addClass(
                    "pick"
                );
                $("#order_" + order_id + " .barcode_" + barcode)
                    .find(".quantity_pick_in")
                    .text(
                        $("#order_" + order_id + " .barcode_" + barcode)
                            .find(".quantity_to_pick_in")
